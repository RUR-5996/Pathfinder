...
//		loadTrajectory(file);
        ...
        leftF.configurePIDVA(Constants.kP, Constants.kI, Constants.kD, 1/Constants.maxSpeed, 0.1); // 0.0 initialization of PID -> play a bit with the values
        rightF.configurePIDVA(Constants.kP, Constants.kI, Constants.kD, 1/Constants.maxSpeed, 0.1); //0.0
 //       Pathfinder.writeToCSV(file, trajectory);
	...
	public void execute() //the problem is somewhere here
	{
        ...
        motorOutputR = rightF.calculate(RobotMap.oneR.getSelectedSensorPosition()); //this one was calculating with inverted encoder
        ...
        System.out.printf("%20f,%20f,%20f,%20f,\n", (motorOutputL+turn), (motorOutputR-turn),  leftRight[0], leftRight[1]);
	}
	...
	public void createTrajectory() { //input is in in and degrees
		waypoints = new Waypoint[] {new Waypoint(0.0, 0.0, 0.0), new Waypoint(1.4, -1.5, Pathfinder.d2r(-15)),new Waypoint(1.8, -1.5, Pathfinder.d2r(0)), new Waypoint(2.8, -0.5, Pathfinder.d2r(-90))};
 ...
 /*       for (int i = 0; i<angles.length; i++)
        ...
        */     
        config = new Trajectory.Config(Trajectory.FitMethod.HERMITE_CUBIC, Trajectory.Config.SAMPLES_HIGH, 0.05, Constants.maxSpeed, 1.5, 60.0);
	...
	public static double[] boundOutput(double left, double right)
	{
		...
		//toReturn[0] = toReturn[0]*0.5;
		//toReturn[1] = toReturn[1]*0.5;
		return toReturn;
	}
